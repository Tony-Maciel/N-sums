...
### Solution
My solution to this problem is to write a meta program in python that creates another program (in fortran) with the desired number of loops. In detail, in this python program (main.py), I: read the integer N, open a new fortran file and write some boilerplate code required for it, declare the roughly N variables to be used in the loops in this fortran file, write each do loop explicitly, write code to make the fortran file save the results of the loops in an output file and finally compile and run this fortran file.
